...
toggleRuneMinLevelContainer('runMultipleSimulations', 'numSimulationsContainer');
    ...
    document.getElementById('startEnhance').addEventListener('click', () => {
...
const protectRuneMinLevel = protectRune ? parseInt(document.getElementById('protectRuneMinLevel').value) : Number.MAX_SAFE_INTEGER;
        ...
        const numSimulations = runMultipleSimulations ? parseInt(document.getElementById('numSimulations').value) : 1;
        let totalAttempts = 0;
...
resultMessage += ` Average guarantee levels: ${averageGarLevelsStr}.`;
 if (boostRune) { resultMessage += `Boost Runes used: ${totalBoostRunesUsed}. Boost runes would cost ${boostRunesDiamondCost} diamonds.`; } if (protectRune) { resultMessage += `Protect Runes used: ${totalProtectRunesUsed}. Protector runes would cost ${protectRunesDiamondCost} diamonds.`; } if (boostRune || protectRune) { resultMessage += `Total diamond cost: ${totalDiamondCost}. This costs around $${(totalDiamondCost / diamondsPerDollar).toFixed(2)}.`;
}
}
